...
<div class="container mt-36 mb-10">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl py-5 px-8 m-auto">
        ...
        <div class="mx-auto block max-w-xl rounded-lg bg-white p-6 shadow-4">
            <form action="{{ route('admin.show.daftaraset.pengembalian.store', ['user_id' => $admin->id, 'peminjaman_id' => $peminjaman->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="lokasi" name="lokasi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                            ...
                            @foreach ($lokasi as $lo)
                                <option value="{{ $lo->id }}" {{ old('lokasi') == $lo->id ? 'selected' : '' }}>{{ $lo->alamat }}</option>
                            @endforeach
                        </select>
                    </div>                      
                    <div class="sm:col-span-2">
                        <label for="keterangan" class="block mb-2 text-sm font-medium text-gray-900">Keterangan</label>
                        <textarea id="keterangan" name="keterangan" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 " placeholder="Tulis Keterangan di sini...">{{ old('keterangan') }}</textarea>                    
                    </div>
                    <div class="sm:col-span-2">
                        <p class="block mb-2 text-sm font-medium text-gray-90">Formulir Pengembalian</p>
                        <a href="{{ route('admin.show.daftaraset.pengembalian.exportformulir', ['user_id' => $admin->id, 'peminjaman_id' => $peminjaman->id]) }}" class="bg-[#008d8d] hover:bg-[#006c6c] text-white font-bold py-1 px-2 rounded text-sm">
                            Download Formulir
                        </a>
                    </div>
                    <div class="sm:col-span-2">
