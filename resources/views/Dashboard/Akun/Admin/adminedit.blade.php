...
<div class="container mt-36 mb-10">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl py-5 px-8 m-auto">
        <div class="flex p-6">
            ...
            <div class="m-auto">
                <h1 class="text-lg font-semibold text-gray-900">Edit Admin</h1>
            </div>
            ...
            <form action="{{ route('admin.update', $admin->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="nama" id="nama" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan nama" value="{{ $admin->name }}" required="">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="email" class="block mb-2 text-sm font-medium text-gray-90">Email</label>
                        <input type="email" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan email" value="{{ $admin->email }}" required="">
                    </div>                   
                    <div class="">
                        ...
                        <select id="divisi" name="divisi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                            ...
                            @foreach ($divisi as $di)
                                <option value="{{ $di->id }}" {{ $di->id == $admin->profile->divisi_id ? 'selected' : '' }}>{{ $di->namaDivisi }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="alamat" id="alamat" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan alamat" value="{{ $admin->profile->alamat }}" required="">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="noTelp" class="block mb-2 text-sm font-medium text-gray-90">No Telp</label>
                        <input type="text" name="noTelp" id="noTelp" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan no telp" value="{{ $admin->profile->noTelp }}" required="">
                    </div>
                    <div class="sm:col-span-2 flex items-center my-auto">
                        <div class="w-1/3 flex mr-4">
                            <img class="img-preview inline-block h-40 w-40 rounded-full object-cover object-center" src="{{ $admin->profile->image ? asset('storage/'. $admin->profile->image) : '' }}">
                        </div>                        
                        <div class="max-w-sm">
                            <label class="block">
                                <input type="hidden" name="oldImage" value="{{ $admin->profile->image }}">
                ...
                <div class="w-full text-center mt-2">
                     <a href="{{ route('admin.password.edit', $admin->id) }}" class="text-blue-600 hover:underline hover:text-primary-dark">Ubah Password</a>
                </div>
            </form>
        </div>
    </div>
