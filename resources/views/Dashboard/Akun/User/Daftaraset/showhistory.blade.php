    ...
    <div class="bg-white max-w-4xl m-auto w-auto rounded-xl shadow-xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <div class="mx-4">
            <div class="flex justify-between">
                <div class="flex">
                    <a href="{{ route('user.show.history', $user->id) }}">
            ...
            <div class="items-center mb-8 lg:block text-center">
                <div class="">
                    @if ($user->profile->image)
                        <img src="{{ asset('storage/'. $user->profile->image) }}" alt="User Image" class="inline-block h-20 w-20 rounded-full">
                    @else
                ...
                <div class="mt-2">
                    ...
                    <h2 class="text-lg font-semibold leading-none text-gray-900 md:text-xl">{{ $user->name }} 
                    </h2>
            ...
            <div class="">
                ...
                @if ($peminjaman->status === 'Diterima')
                    <div class="mt-2 flex">
                        ...
                        <a href="{{ route('user.show.daftaraset.exportbuktiformulir', ['user_id' => $user->id, 'peminjaman_id' => $peminjaman->id]) }}" class="bg-[#008d8d] hover:bg-[#006c6c] text-white font-bold py-1 px-2 rounded text-sm">
                ...
                @elseif ($peminjaman->status === 'Diproses')
                    <div class="mt-2 flex">
                        ...
                        <a href="{{ route('user.show.daftaraset.exportformulir', ['user_id' => $user->id, 'peminjaman_id' => $peminjaman->id]) }}" class="bg-[#008d8d] hover:bg-[#006c6c] text-white font-bold py-1 px-2 rounded text-sm">
                            Download Formulir
                        </a>
                    </div>
                    <div class="mt-2 flex">
                        <p class="w-60">Upload Formulir</p>
                        <a href="{{ route('user.show.daftaraset.uploadformulir', ['user_id' => $user->id, 'peminjaman_id' => $peminjaman->id]) }}" class="bg-[#008d8d] hover:bg-[#006c6c] text-white font-bold py-1 px-2 rounded text-sm">
                            Upload Formulir
                        </a>
                    </div>
                @endif
