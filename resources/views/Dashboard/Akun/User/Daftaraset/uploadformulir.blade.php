...
<div class="container mt-36 mb-10">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14 m-auto">
        <div class="flex p-6">
            <a href="{{ route('user.show.daftaraset.showhistory', ['user_id' => $user->id, 'peminjaman_id' => $peminjaman->id]) }}">
            ...
            <div class="m-auto">
                <h1 class="text-lg font-semibold text-gray-900">Upload Formulir</h1>
            </div>
            
        </div>
        <div class="mx-auto block max-w-xl rounded-lg bg-white p-6 shadow-4">
            <form action="{{ route('user.show.daftaraset.uploadformulir.update', ['user_id' => $user->id, 'peminjaman_id' => $peminjaman->id]) }}" method="post" enctype="multipart/form-data">
                @csrf
