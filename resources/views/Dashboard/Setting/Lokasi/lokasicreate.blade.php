    ...
    <div class="p-4 border-2">
       <div class="bg-white w-auto rounded-b-xl shadow-xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div class="flex p-6">
            ...
            <div class="m-auto">
                <h1 class="text-lg font-semibold text-gray-900">Create lokasi</h1>
            </div>
            
        </div>
        <div class="mx-auto block max-w-xl rounded-lg bg-white p-6 shadow-4">
            <form action="{{ route('setting.store.lokasi') }}" method="post" enctype="multipart/form-data">
                @csrf
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="kodeLokasi" class="block mb-2 text-sm font-medium text-gray-90 ">Kode Lokasi</label>
                        <input type="text" name="kodeLokasi" id="kodeLokasi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 uppercase" placeholder="Masukkan Kode Lokasi" required>
                        @error('kodeLokasi')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="alamat" id="alamat" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan Alamat" required>
                        @error('alamat')
                            <p class="text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
