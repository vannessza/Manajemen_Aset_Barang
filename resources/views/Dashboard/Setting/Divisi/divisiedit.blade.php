    ...
    <div class="p-4 border-2">
       <div class="bg-white w-auto rounded-b-xl shadow-xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        ...
        <div class="mx-auto block max-w-xl rounded-lg bg-white p-6 shadow-4">
            <form action="{{ route('setting.update.divisi', $divisi->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="_method" value="PATCH">
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="kodeDivisi" class="block mb-2 text-sm font-medium text-gray-90 ">Kode Divisi</label>
                        <input type="text" name="kodeDivisi" id="kodeDivisi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 uppercase" placeholder="Masukkan Kode Divisi" required="" value="{{ $divisi->kodeDivisi }}">
                        @error('kodeDivisi')
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="text" name="namaDivisi" id="namaDivisi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan Nama Divisi" required="" value="{{ $divisi->namaDivisi }}">
                        @error('namaDivisi')
                            <p class="text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
