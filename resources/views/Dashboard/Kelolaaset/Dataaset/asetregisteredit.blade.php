...
<div class="container mt-36 mb-10">
    <div class="bg-white rounded-xl shadow-xl max-w-2xl py-5 px-8 m-auto">
        <div class="flex p-6">
            ...
            <div class="m-auto">
                <h1 class="text-lg font-semibold text-gray-900">Edit Aset Register</h1>
            </div>
            
        </div>
        <div class="mx-auto block max-w-xl rounded-lg bg-white p-6 shadow-4">
            <form action="{{ route('dataaset.update', $aset->id ) }}" method="post" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="_method" value="PATCH">
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label for="aset" class="block mb-2 text-sm font-medium text-gray-90">Aset</label>
                        <input type="text" name="aset" id="aset" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan nama aset register" required value="{{ $aset->namaAset }}">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="jenisAset" class="block mb-2 text-sm font-medium text-gray-90">Jenis Aset register</label>
                        <input type="text" name="jenisAset" id="jenisAset" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan jenis aset register" required value="{{ $aset->jenisAset }}">
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <select id="klasifikasiAset" name="klasifikasiAset" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                            ...
                            <option value="Terbatas" {{ $aset->klasifikasiAset === 'Terbatas' ? 'selected' : '' }}>Terbatas</option>
                            <option value="Rahasia" {{ $aset->klasifikasiAset === 'Rahasia' ? 'selected' : '' }}>Rahasia</option>
                            <option value="Internal" {{ $aset->klasifikasiAset === 'Internal' ? 'selected' : '' }}>Internal</option>
                            <option value="Publik" {{ $aset->klasifikasiAset === 'Publik' ? 'selected' : '' }}>Publik</option>
                        </select>
                    </div>
                    <div class="sm:col-span-2">
                        ...
                        <select id="ciaLevel" name="ciaLevel" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                            ...
                            <option value="Low"{{ $aset->ciaLevel === 'Low' ? 'selected' : '' }}>Low</option>
                            <option value="Medium" {{ $aset->ciaLevel === 'Medium' ? 'selected' : '' }}>Medium</option>
                            <option value="High" {{ $aset->ciaLevel === 'High' ? 'selected' : '' }}>High</option>
                        </select>
                    </div>
                    <div class="" data-hs-input-number="">
                        ...
                        <div class="w-full flex justify-between items-center gap-x-5">
                            <div class="grow">
                                <input id="nilaiRisiko" name="nilaiRisiko" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" type="text" required value="{{ $aset->nilaiRisiko }}">
                            </div>
                    ...
                    <div class="" data-hs-input-number="">
                        ...
                        <div class="w-full flex justify-between items-center gap-x-5">
                            <div class="grow">
                                <input id="masaRetensi" name="masaRetensi" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" type="text" required value="{{ $aset->masaRetensi }}">
                            </div>
                            <div class="flex justify-end items-center gap-x-1.5">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea id="detailAset" name="detailAset" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 " placeholder="Tuliskan detail aset register" value="{{ $aset->detailAset }}">{{ $aset->detailAset }}</textarea>                    
                    </div>
