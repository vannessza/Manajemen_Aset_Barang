          ...
          <td class="whitespace-nowrap">
            <div class="px-6 py-1.5">
              @if($pe->status === 'Disetujui')
                <a class="inline-flex items-center gap-x-1 text-sm text-slate-500 hover:text-slate-700 disabled:opacity-50 decoration-2 hover:underline font-medium" href="{{ route('penghancuran.show.history', $pe->id) }}">
                  Detail
                </a>
                <a class="inline-flex items-center gap-x-1 text-sm text-red-500 hover:text-red-800 decoration-2 hover:underline font-medium" href="{{ route('penghancuran.delete', $pe->id) }}">
                  Delete
                </a>
              @else
              <a class="inline-flex items-center gap-x-1 text-sm text-green-500 hover:text-green-800 disabled:opacity-50 decoration-2 hover:underline font-medium" href="{{ route('penghancuran.edit', $pe->id) }}">
                Edit
              </a>
                <a class="inline-flex items-center gap-x-1 text-sm text-red-500 hover:text-red-800 decoration-2 hover:underline font-medium" href="{{ route('penghancuran.delete', $pe->id) }}">
                  Delete
    ...
    <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200">
      <div>
        <p class="text-sm text-gray-600">
          <span class="font-semibold text-gray-800">{{ count($penghancuran) }}</span> results
        </p>
      </div>

      <div class="">
        <div class="inline-flex gap-x-2">
            @if ($penghancuran->previousPageUrl())
                <a href="{{ $penghancuran->previousPageUrl() }}" class="py-1.5 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50">
            ...
            @if ($penghancuran->nextPageUrl())
                <a href="{{ $penghancuran->nextPageUrl() }}" class="py-1.5 px-2 inline-flex items-center gap-x-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-800 shadow-sm hover:bg-gray-50">
                    Next
