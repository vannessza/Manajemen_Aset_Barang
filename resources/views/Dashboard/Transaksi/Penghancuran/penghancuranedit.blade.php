                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div class="">
                        ...
                        <select id="namaAset" name="namaAset" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5" required>
                            ...
                            <!-- Opsi Nama Aset akan diisi oleh JavaScript -->
                        </select>
                    </div>
                    <div class="sm:col-span-2">
                        <label for="tipePemusnahan" class="block mb-2 text-sm font-medium text-gray-90">Tipe Pemusnahan</label>
                        <input type="text" name="tipePemusnahan" id="tipePemusnahan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Masukkan Tipe Pemusnahan" required value="{{ $penghancuran->tipePemusnahan }}">
                    </div>
                    <div class="sm:col-span-2">
                        <label for="tglPemusnahan" class="block mb-2 text-sm font-medium text-gray-90">Tanggal Pemusnahan</label>
                        <input type="date" name="tglPemusnahan" id="tglPemusnahan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " required value="{{ $penghancuran->tglPemusnahan }}">
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        var selectedNamaAset = "{{ $penghancuran->nama_aset }}"; // Mendapatkan nilai sebelumnya

        function updateNamaAsetDropdown() {
            var selectedAsetId = asetDropdown.value;
            ...
            @foreach ($aset as $as)
                if ({{ $as->id }} == selectedAsetId) {
                    @foreach ($as->asetDetail as $detail)
                        ...
                        if ("{{ $detail->namaAset }}" == selectedNamaAset) {
        ...
        updateNamaAsetDropdown();

        // Tambahkan event listener untuk dropdown aset
        asetDropdown.addEventListener('change', function() {
            updateNamaAsetDropdown();
        });
    });
</script>
