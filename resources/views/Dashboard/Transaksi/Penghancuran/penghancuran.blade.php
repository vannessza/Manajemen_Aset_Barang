...
<div class="relative max-w-xs ml-2">
    <form method="GET" action="{{ route('penghancuran.datapenghancuran') }}">
          ...
          <td class="whitespace-nowrap">
            <div class="px-6 py-1.5">
              @if($pe->status === 'Disetujui')
                <a class="inline-flex items-center gap-x-1 text-sm text-slate-500 hover:text-slate-700 disabled:opacity-50 decoration-2 hover:underline font-medium" href="{{ route('penghancuran.show', $pe->id) }}">
                  Detail
                </a>
              @else
