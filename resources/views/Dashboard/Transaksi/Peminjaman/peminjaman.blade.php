@extends('dashboard.transaksi.peminjaman.index')

@section('peminjaman')
@if(count($peminjaman) > 0)
<div class="relative max-w-xs ml-2 mb-4">
  <form method="GET" action="{{ route('peminjaman.history') }}">
...
<table class="min-w-full divide-y divide-gray-200">
    ...
    <tbody class="divide-y divide-gray-200">
      ...
      <tr>
        ...
        <td class="whitespace-nowrap">
          <div class="px-6 py-3">
            <span class="text-sm text-gray-500" style="max-width: 150px;">{{ $pe->kodePeminjaman }}</span>
          </div>
        </td>
        <td class="whitespace-nowrap">
          <div class="px-6 py-3">
      ...
      <td class="whitespace-nowrap">
        ...
        <td class="whitespace-nowrap">
          <div class="px-6 py-1.5">
            <a class="inline-flex items-center gap-x-1 text-sm text-slate-500 hover:text-slate-700 disabled:opacity-50 decoration-2 hover:underline font-medium" href="{{ route('peminjaman.show', $pe->id) }}">
              Detail
            </a>
            {{-- <a class="inline-flex items-center gap-x-1 text-sm text-green-500 hover:text-green-800 disabled:opacity-50 decoration-2 hover:underline font-medium" href="{{ route('peminjaman.edit', $pe->id) }}">
              Edit
            </a> --}}
            <a class="inline-flex items-center gap-x-1 text-sm text-red-500 hover:text-red-800 decoration-2 hover:underline font-medium" href="{{ route('peminjaman.delete', $pe->id) }}">
              Delete
            </a>
          </div>
        </td>
  ...
  <div class="alert alert-info mt-10 text-center">
      Belum ada History yang tersedia.
  </div>
  @endif
@endsection